...
import { getMenuTree, addMenuApi, updateMenuApi, logicDeleteMenuApi } from '@/api/system/menu'
...
const rules = ref({
  ...
  perms: [
    ...
    { pattern: /^[a-zA-Z0-9:-]+$/, message: '权限标识只能包含字母、数字、英文冒号', trigger: 'blur' }
  ],
  path: [
    {
  ...
  sort: [
    { type: 'number', min: 0, max: 999, message: '排序必须在0~999之间' }
  ]
})
    ...
    <el-table ref="tableRef" :data="tableData" border stripe row-key="id" tree-props="{ children: 'children' }">
      ...
      <el-table-column label="操作" width="240" align="center">
        <template #default="{ row }">
          ...
          <el-button size="small" type="success" @click="openAddChild(row)">新增下级</el-button>
  ...
  <el-dialog v-model="dialogVisible" title="菜单编辑" width="650px" @close="resetForm">
    <el-form ref="menuFormRef" :model="menuForm" label-width="110px" :rules="rules">
      <el-form-item label="上级菜单">
        <el-tree-select v-model="menuForm.parentId" :data="menuTree" placeholder="顶级菜单" check-strictly node-key="id"
          :props="{ label: 'menuName', children: 'children' }" value-format="number" />
      </el-form-item>
