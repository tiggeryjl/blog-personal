...
const getAllRoleList = async () => {
  try {
    ...
    if (result.code == 200) {
      ...
      roleAssignOptions.value = realRoles.filter(
        (role) => role.roleKey !== ROLE_KEY_SUPER,
      );
    }
  } catch (error) {
...
const handleBatchDelete = async () => {
  ...
  ElMessageBox.confirm("您确认要删除选中的所有用户吗?", "提示", {
    confirmButtonText: "确认",
    cancelButtonText: "取消",
    type: "warning",
  }).then(async () => {
...
<template>
  <div
    ...
    style="padding: 20px; max-width: 100%; overflow-x: hidden"
  >
    <div
      class="header-row"
      style="
          ...
          <template #default="{ row }">
            ...
            >
              <el-switch
                ...
                inactive-text="禁用"
                style="
                  --el-switch-on-color: #6ecc54;
                  --el-switch-off-color: #fd4e4e;
                "
    ...
    <el-dialog v-model="roleDialogVisible" title="分配用户角色" width="450px">
      <div v-if="currentRow">
        ...
        >
          <el-option
            v-for="item in roleAssignOptions"
            :key="item.id"
            :label="item.roleName"
            :value="item.id"
          />
